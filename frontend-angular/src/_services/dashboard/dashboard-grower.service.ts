...
import { httpGetOptions } from '../../_shared/httpPostOptions';
...
export class DashboardGrowerService {
  ...
  logSensorModuleLevels = this.roomsSource.asObservable();

  private logSensorReservoirsSource: BehaviorSubject<
    LogSensorReservoir[]
  > = new BehaviorSubject<LogSensorReservoir[]>([]);
  logSensorReservoirs = this.roomsSource.asObservable();

  private logSensorRoomsSource: BehaviorSubject<
    LogSensorRoom[]
  > = new BehaviorSubject<LogSensorRoom[]>([]);
  logSensorRooms = this.roomsSource.asObservable();

  constructor(private httpClient: HttpClient) {}

  fetchAllDashboardData(): Promise<any> {
    return this.httpClient
      .get<any>(DASHBOARD_GROWER_API, httpGetOptions)
      .toPromise();
  }

  populateDashboard() {
    this.fetchAllDashboardData().then((response: HttpResponse<any>) => {
